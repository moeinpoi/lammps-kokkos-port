...
#include "atom_vec_x0image_kokkos.h"
...
void AtomVecX0ImageKokkos::grow(int n)
{
...
struct AtomVecX0ImageKokkos_PackBorder {
  typedef DeviceType device_type;
  typedef ArrayTypes<DeviceType> AT;
  ...
  AtomVecX0ImageKokkos_PackBorder(
      ...
      _x(x),_tag(tag),_type(type),_mask(mask),_x0(x0),
                               ...
                               int pbc_flag, int *pbc, ExecutionSpace space)
{
  ...
  if (pbc_flag != 0) {
    ...
    if (space==Host) {
      AtomVecX0ImageKokkos_PackBorder<LMPHostType,1> f(
        buf.view<LMPHostType>(), k_sendlist.view<LMPHostType>(),
        iswap,h_x,h_tag,h_type,h_mask,h_x0,dx,dy,dz);
      Kokkos::parallel_for(n,f);
    } else {
      AtomVecX0ImageKokkos_PackBorder<LMPDeviceType,1> f(
        buf.view<LMPDeviceType>(), k_sendlist.view<LMPDeviceType>(),
        iswap,d_x,d_tag,d_type,d_mask,d_x0,dx,dy,dz);
      Kokkos::parallel_for(n,f);
    }
    ...
    dx = dy = dz = 0;
    if (space==Host) {
      AtomVecX0ImageKokkos_PackBorder<LMPHostType,0> f(
        buf.view<LMPHostType>(), k_sendlist.view<LMPHostType>(),
        iswap,h_x,h_tag,h_type,h_mask,h_x0,dx,dy,dz);
      Kokkos::parallel_for(n,f);
    } else {
      AtomVecX0ImageKokkos_PackBorder<LMPDeviceType,0> f(
        buf.view<LMPDeviceType>(), k_sendlist.view<LMPDeviceType>(),
        iswap,d_x,d_tag,d_type,d_mask,d_x0,dx,dy,dz);
      Kokkos::parallel_for(n,f);
    }
...
struct AtomVecX0ImageKokkos_UnpackBorder {
  typedef DeviceType device_type;
  typedef ArrayTypes<DeviceType> AT;
  ...
  AtomVecX0ImageKokkos_UnpackBorder(
      const typename AT::t_xfloat_2d_const &buf,
      typename AT::t_x_array &x,
      typename AT::t_tagint_1d &tag,
      typename AT::t_int_1d &type,
      typename AT::t_int_1d &mask,
      typename AT::t_x_array &x0,
      const int& first):
    _buf(buf),_x(x),_tag(tag),_type(type),_mask(mask),_x0(x0),_first(first) {
  };
  ...
  void operator() (const int& i) const {
      ...
      _x0(i+_first,0) = _buf(i,6);
      _x0(i+_first,1) = _buf(i,7);
      _x0(i+_first,2) = _buf(i,8);
  }
};
...
void AtomVecX0ImageKokkos::unpack_border_kokkos(const int &n, const int &first,
  ...
  if (space==Host) {
    struct AtomVecX0ImageKokkos_UnpackBorder<LMPHostType>
      f(buf.view<LMPHostType>(),h_x,h_tag,h_type,h_mask,h_x0,first);
    Kokkos::parallel_for(n,f);
  } else {
    struct AtomVecX0ImageKokkos_UnpackBorder<LMPDeviceType>
      f(buf.view<LMPDeviceType>(),d_x,d_tag,d_type,d_mask,d_x0,first);
    Kokkos::parallel_for(n,f);
  }
  atomKK->modified(space,X_MASK|TAG_MASK|TYPE_MASK|MASK_MASK|X0_MASK);
}

/* ---------------------------------------------------------------------- */

template<class DeviceType>
struct AtomVecX0ImageKokkos_PackExchangeFunctor {
  typedef DeviceType device_type;
  typedef ArrayTypes<DeviceType> AT;
  typename AT::t_x_array_randomread _x;
  ...
  AtomVecX0ImageKokkos_PackExchangeFunctor(
  ...
  void operator() (const int &mysend) const {
    ...
    _buf(mysend,11) = _x0(i,0);
    _buf(mysend,12) = _x0(i,1);
    _buf(mysend,13) = _x0(i,2);
...
int AtomVecX0ImageKokkos::pack_exchange_kokkos(const int &nsend,DAT::tdual_xfloat_2d &k_buf,
                                              ...
                                              ExecutionSpace space)
{
  ...
  if (space == Host) {
    AtomVecX0ImageKokkos_PackExchangeFunctor<LMPHostType>
      f(atomKK,k_buf,k_sendlist,k_copylist);
    Kokkos::parallel_for(nsend,f);
    return nsend*size_exchange;
  } else {
    AtomVecX0ImageKokkos_PackExchangeFunctor<LMPDeviceType>
      f(atomKK,k_buf,k_sendlist,k_copylist);
    Kokkos::parallel_for(nsend,f);
    return nsend*size_exchange;
  }
...
struct AtomVecX0ImageKokkos_UnpackExchangeFunctor {
  typedef DeviceType device_type;
  typedef ArrayTypes<DeviceType> AT;
  typename AT::t_x_array _x;
  ...
  AtomVecX0ImageKokkos_UnpackExchangeFunctor(
    const AtomKokkos* atom,
    const typename AT::tdual_xfloat_2d buf,
  ...
  void operator() (const int &myrecv) const {
    ...
    if (x >= _lo && x < _hi) {
      ...
      _x0(i,0) = _buf(myrecv,11);
      _x0(i,1) = _buf(myrecv,12);
      _x0(i,2) = _buf(myrecv,13);
    }
    if (OUTPUT_INDICES)
      _indices(myrecv) = i;
  }
...
/* ---------------------------------------------------------------------- */
int AtomVecX0ImageKokkos::unpack_exchange_kokkos(DAT::tdual_xfloat_2d &k_buf, int nrecv, int nlocal,
                                                ...
                                                DAT::tdual_int_1d &k_indices)
{
  ...
  if (space == Host) {
    if (k_indices.h_view.data()) {
      ...
      AtomVecX0ImageKokkos_UnpackExchangeFunctor<LMPHostType,1> f(atomKK,k_buf,k_count,k_indices,dim,lo,hi);
      Kokkos::parallel_for(nrecv/size_exchange,f);
    } else {
      k_count.h_view(0) = nlocal;
      AtomVecX0ImageKokkos_UnpackExchangeFunctor<LMPHostType,0> f(atomKK,k_buf,k_count,k_indices,dim,lo,hi);
      Kokkos::parallel_for(nrecv/size_exchange,f);
    }
  } else {
    if (k_indices.h_view.data()) {
      ...
      AtomVecX0ImageKokkos_UnpackExchangeFunctor<LMPDeviceType,1>
      ...
      k_count.modify<LMPHostType>();
      k_count.sync<LMPDeviceType>();
      AtomVecX0ImageKokkos_UnpackExchangeFunctor<LMPDeviceType,0>
        f(atomKK,k_buf,k_count,k_indices,dim,lo,hi);
      Kokkos::parallel_for(nrecv/size_exchange,f);
      k_count.modify<LMPDeviceType>();
      k_count.sync<LMPHostType>();
...
void AtomVecX0ImageKokkos::sync(ExecutionSpace space, unsigned int mask)
{
  if (space == Device) {
...
void AtomVecX0ImageKokkos::modified(ExecutionSpace space, unsigned int mask)
{
  if (space == Device) {
...
void AtomVecX0ImageKokkos::sync_overlapping_device(ExecutionSpace space, unsigned int mask)
{
  if (space == Device) {
