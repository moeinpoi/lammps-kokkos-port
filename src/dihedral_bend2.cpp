...
DihedralBend2::DihedralBend2(LAMMPS *lmp) : Dihedral(lmp)
{
  writedata = 1;
}
...
void DihedralBend2::compute(int eflag, int vflag)
{
  ...
  for (n = 0; n < ndihedrallist; n++) {
    ...
    // 3-2 distance
    d32x = x0[i3][0] - x0[i2][0];
    d32y = x0[i3][1] - x0[i2][1];
    d32z = x0[i3][2] - x0[i2][2];
    //domain->minimum_image(d32x,d32y,d32z);

    // 3-4 distance
    d34x = x0[i3][0] - x0[i4][0];
    d34y = x0[i3][1] - x0[i4][1];
    d34z = x0[i3][2] - x0[i4][2];
    //domain->minimum_image(d34x,d34y,d34z);

    // 2-4 distance
    d24x = x0[i2][0] - x0[i4][0];
    d24y = x0[i2][1] - x0[i4][1];
    d24z = x0[i2][2] - x0[i4][2];
    //domain->minimum_image(d24x,d24y,d24z);

    // 1-4 distance
    d14x = x0[i1][0] - x0[i4][0];
    d14y = x0[i1][1] - x0[i4][1];
    d14z = x0[i1][2] - x0[i4][2];
    ...
    costheta0 = (n1x*n2x + n1y*n2y + n1z*n2z)/nn; 
...
void DihedralBend2::coeff(int narg, char **arg)
{
...
void DihedralBend2::write_restart(FILE *fp)
{
...
void DihedralBend2::read_restart(FILE *fp)
{
  allocate();
...
void DihedralBend2::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->ndihedraltypes; i++)
    fprintf(fp,"%d %g %g\n",i,k[i],theta0[i]);
}
